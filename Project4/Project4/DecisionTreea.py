...
def makeSubtrees(remainingAttributes,examples,attributeValues,className,defaultLabel,setScoreFunc,gainFunc):
    ...
    same = True
    for example in examples:
        same = same & (example[className]==firstClassVal)
        if not same:
            break
    if same:
        return LeafNode(firstClassVal)
    if not remainingAttributes:
        root = LeafNode(getMostCommonClass(example, className))
        return root
    ...
    for attr in remainingAttributes:
        if gainFunc(examples, attr, attributeValues[attr],className) > gainFunc(examples, argmax, attributeValues[argmax], className):
            argmax = attr
    root = Node(argmax)
    curAttr = list(remainingAttributes)
    curAttr.remove(argmax)
    for attrValue in attributeValues[argmax]:
        cur = getPertinentExamples(examples, argmax, attrValue)
        root.children[attrValue] = makeSubtrees(curAttr,cur, attributeValues, className, getMostCommonClass(examples, className), setScoreFunc, gainFunc)
    return root
...
def makePrunedSubtrees(remainingAttributes,examples,attributeValues,className,defaultLabel,setScoreFunc,gainFunc,q):
    ...
    attrdict={}
    for key in attrcnt.keys():
        subsum=0
        for item in attrcnt[key].keys():
            subsum+=attrcnt[key][item]
        attrdict[key]=subsum
    ...
    for key in attrcnt.keys():
        chii=0
        for item in attrcnt[key].keys():
            pi=attrcnt[key][item]*1.0
            pih=(classcnt[item]/(len(examples)*1.0))*attrdict[key]
