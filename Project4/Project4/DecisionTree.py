...
def getAttributeCounts(examples,attrName,attrValues,className):
    ...
    for attrValue in attrValues:
        cur = getPertinentExamples(examples,attrName, attrValue)
        attributeCounts[attrValue] = getClassCounts(cur, className)
...
def setEntropy(classCounts):
    ...
    #YOUR CODE HERE
    entropy = 0
    for cnt in classCounts:
        cnt = float(cnt)/sum(classCounts)
        entropy -= (cnt*log(cnt,2))
    return entropy
...
def remainder(examples,attrName,attrValues,className):
    ...
    for attrValue in attrValues:
        cur = getPertinentExamples(examples, attrName, attrValue)
        kcnt = getClassCounts(cur, className).values()
        rm += float(sum(kcnt))/sum(classcnt.values())*setEntropy(kcnt)
    return rm
...
def infoGain(examples,attrName,attrValues,className):
    ...
    #YOUR CODE HERE
    classcnt = getClassCounts(examples, className)
    return setEntropy(classcnt.values()) - remainder(examples, attrName, attrValues, className)
...
def giniGain(examples,attrName,attrValues,className):
    ...
    for attrValue in attrValues:
        cur = getPertinentExamples(examples, attrName, attrValue)
        classcnt = getClassCounts(cur, className)
        gain += float(len(cur))/len(examples)*giniIndex(classcnt.values())
    if gain == 0:
        return maxint
    return 1.0/gain
...
def makeSubtrees(remainingAttributes,examples,attributeValues,className,defaultLabel,setScoreFunc,gainFunc):
    ...
    for attr in remainingAttributes:
        if gainFunc(examples,attr,attributeValues[attr],className) > gainmax:
            gainmax = gainFunc(examples,attr,attributeValues[attr],className)
            argmax = attr
    ...
    for value in attributeValues[argmax]:
        ...
        root.children[value] = makeSubtrees(newremainattr, cur, attributeValues, className, getMostCommonClass(examples,className), setScoreFunc,gainFunc)    
    return root
...
def makePrunedSubtrees(remainingAttributes,examples,attributeValues,className,defaultLabel,setScoreFunc,gainFunc,q):
    ...
    attrdict = getAttributeCounts(examples, argmax, attributeValues[argmax], className)
    dicta = {}
    ...
    for value in attributeValues[argmax]:
        cur = getPertinentExamples(examples,argmax,value)        
        root.children[value] = makePrunedSubtrees(newremainattr, cur, attributeValues, className, getMostCommonClass(examples,className), setScoreFunc,gainFunc,q)    
    return root
