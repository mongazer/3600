...
def consistent(assignment, csp, var, value):
	...
	for cons in csp.binaryConstraints:
		if cons.affects(var):
			var2 = cons.otherVariable(var)
			if assignment.isAssigned(var2):
				if not cons.isSatisfied(assignment.assignedValues[var2], value):
					return False
...
def recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod):
	...
	if assignment.isComplete():
		return assignment
	curvar = selectVariableMethod(assignment, csp)
	for val in orderValuesMethod(assignment, csp, curvar):
		if consistent(assignment, csp, curvar, val):
			assignment.assignedValues[curvar] = val
			if recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod) != None:
				return assignment
			assignment.assignedValues[curvar] = None
	return None
...
def minimumRemainingValuesHeuristic(assignment, csp):
	...
	for var in domains:
		if not assignment.isAssigned(var):
			deg = 0
			for cons in csp.binaryConstraints:
				if cons.affects(var) and not assignment.isAssigned(cons.otherVariable(var)):
					deg = deg + 1
			rv = len(domains[var])
			if rv < minrv:
				minrv = rv
				nextVar = var
			if rv==minrv and deg>mindeg:
				mindeg = deg
				nextVar = var
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
	...
	"""Question 3"""
	"""YOUR CODE HERE"""
	priorityval = Queue.PriorityQueue()
	for val in values:
		num = 0
		for cons in csp.binaryConstraints:
			if cons.affects(var):
				for remainval in assignment.varDomains[cons.otherVariable(var)]:
					if not cons.isSatisfied(remainval,val):
						num = num + 1
		priorityval.put((num, val))

	values = []
	while not priorityval.empty():
		values.append(priorityval.get()[1])
	...
	for cons in csp.binaryConstraints:
		if cons.affects(var):
			...
			if not assignment.isAssigned(var2):
				temp = domains[var2]
				for val in temp:
					if not cons.isSatisfied(val, value):
						if len(domains[var2]) < 2:
							return None
						else:
							inferences.add((var2, value))
	for inf in inferences:
		domains[inf[0]].remove(inf[1])
	return inferences
...
def recursiveBacktrackingWithInferences(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod):
	...
	for val in orderValuesMethod(assignment, csp, var):
		if consistent(assignment, csp, var, val):
			...
			if inferences != None:
				rt =  recursiveBacktrackingWithInferences(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod)
			else:
				continue
			if (rt):
				return rt
...
def maintainArcConsistency(assignment, csp, var, value):
	...
	for cons in csp.binaryConstraints:
		if cons.affects(var):
			arc.append((var, cons.otherVariable(var)))
	while not len(arc) == 0:
		var1, var2 = arc[0]
		del arc[0]
		curcon = None
		for cons in csp.binaryConstraints:
			if cons.affects(var1) and cons.affects(var2):
				curcon = cons
		infset = revise(assignment, csp, var1, var2, curcon)
		if infset == None:
		...
		if len(infset) != 0:
			for cons in csp.binaryConstraints:
				if cons.affects(var2):
					arc.append((var2, cons.otherVariable(var2)))
		inferences = set(inferences.union(infset))
	return inferences
...
def AC3(assignment, csp):
	...
	for cons in csp.binaryConstraints:
		var1 = cons.var1
		var2 = cons.var2
		arc.append((var1, var2, cons))
		arc.append((var2, var1, cons))
	while len(arc) != 0:
		var1, var2, curcon = arc[0]
		del arc[0]
		infset = revise(assignment, csp, var1, var2, curcon)
		if infset == None:
			return None
		if len(infset) != 0:
			for cons in csp.binaryConstraints:
				if cons.affects(var2):
					arc.append((var2, cons.otherVariable(var2), cons))
