...
def getExtraData(fileString ="tic-tac-toe.txt", limit=100000 ):
    """
    returns limit # of examples from file passed as string
    """
    examples=[]
    attrValues={}
    data = open(fileString)
    attrs = ['top-left-square','top-middle-square','top-right-square','middle-left-square','middle-middle-square','middle-right-square','bottom-left-square','bottom-middle-square','bottom-right-square']
    attr_values = []
    for i in range(9):
        attr_values.append(['x','o','b'])

    dicta = {'x':getList(1,3),'o':getList(2,3),'b':getList(3,3)}

    attrNNList=[]
    for i in range(9):
        tuplea = (attrs[i], dicta)
        attrNNList.append(tuplea)    
...
def buildExamplesFromExtraData(size=200):
    ...
    #print "car data list : " + str(carDataList)
    tests = len(Data)-size
    DataTestList = [DataTrainList.pop(random.randint(0,tests+size-t-1)) for t in xrange(tests)]
