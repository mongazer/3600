...
class Perceptron(object):
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        """
        """YOUR CODE"""
        x=self.sigmoidActivation(inActs);
        return x*(1-x)
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for actnum in xrange(len(inActCpy)):
            self.weights[actnum]+=inActCpy[actnum]*alpha*delta
            totalModification+=abs(inActCpy[actnum]*alpha*delta)
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            ...
            for perceptron in layer:
                layerlist.append(perceptron.sigmoidActivation(a[len(a)-1]))
            a.append(layerlist)
        return a
        ...
        for example in examples:#for each example
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    ...
                    delta = gPrime*sum(nextLayer[nextNeuNum].weights[neuronNum + 1] * deltas[0][nextNeuNum] for nextNeuNum in xrange(len(nextLayer)))
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    for expIn, expOut in examplesTest:
        output = nnet.feedForward(expIn)[-1]
        flag=True
        for idx in xrange(len(output)):
            if round(output[idx]) != expOut[idx]:
                flag=False
        if flag:
            testCorrect+=1
        else:
