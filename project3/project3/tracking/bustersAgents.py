...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        potentialGhostPos = util.Counter()
        for dis in livingGhostPositionDistributions:
            maxprob = dis.argMax()
            potentialGhostPos[maxprob] = -1*self.distancer.getDistance(pacmanPosition, maxprob)
        closetGhost = potentialGhostPos.argMax()
        for move in legal:
            if self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, move), closetGhost) < self.distancer.getDistance(pacmanPosition, closetGhost):
                return move