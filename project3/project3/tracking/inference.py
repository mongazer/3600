...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for pt in self.particles:
                trueDistance = util.manhattanDistance(pt, pacmanPosition)
                weights[pt] += emissionModel[trueDistance]
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            updatedPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            updatedPtList.append(util.sampleFromCounter(updatedPosDist))
        self.particles = updatedPtList
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        ghostTup = list(itertools.permutations(self.legalPositions, r = ghostNum))
        random.shuffle(ghostTup)
        for i in range(self.numParticles):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for i in range(ghostNum):
            if noisyDistances[i] != None:
                ...
                jailedghostls.append(i)
        for pt in particles:
            prob = 1.0
            for ghost in jailedghostls:
                helper = self.getParticleWithGhostInJail(pt, ghost)
                pt = helper
            for i in range(ghostNum):
                if i in jailedghostls:
                    pass
                else:
                    prob = emissionModels[i][util.manhattanDistance(pt[i], pacmanPosition)]*prob
            count[pt] += prob
        self.beliefs = count
        ...
        if sum([i for i in count.values()]) != 0:
            ...
            for j in range(ptlen):
                particles[j] = util.sample(beliefs)
        if sum([i for i in count.values()]) == 0:
            self.initializeParticles()
